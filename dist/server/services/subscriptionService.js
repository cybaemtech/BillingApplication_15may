...
async function getPlanFeatureKeys(planId) {
  ...
  `.then((result) => result.recordset);
    return rows.map((row) => String(row.feature_key || "")).filter(Boolean);
}
...
export async function getTenantSubscriptionContext(tenantId) {
...
export async function updateTenantPlan(tenantId, planName) {
