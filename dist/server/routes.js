...
import { authenticate, signToken, resolveEffectiveRole } from "./auth.js";
...
async function requireCybaemtechSuperAdmin(req, res) {
...
function canManageTenantRole(currentRole, targetRole) {
...
async function getTenantRoleNames(tenantId) {
  ...
  `.then((result) => result.recordset).catch(() => []);
    if (rows.length > 0) {
        return rows.map((row) => String(row.name || "").trim().toLowerCase()).filter(Boolean);
    }
...
async function assertTenantOwnership(table, id, tenantId, label) {
    if (!id)
        return;
    if (!TENANT_OWNED_TABLES.has(table))
        throw new Error("Unsupported tenant ownership check");
    const result = await runRawQuery(`SELECT TOP 1 id FROM dbo.${table} WHERE id = @id AND tenant_id = @tenant_id`, { id, tenant_id: tenantId });
    if (!result.recordset[0])
        throw new Error(`${label} does not belong to this organization`);
}
function getSubscriptionSections(plan) {
    return [
        {
            key: "user_management",
...
function getPlanModuleMatrix(plan) {
...
async function runRawQuery(sqlText, inputs = {}) {
...
function getDocumentTableName(docType) {
    const tableNames = {
...
function getDocumentIdentifierColumn(docType) {
    const columnNames = {
...
async function sendPaginatedResults(req, res, dataQuery, countQuery, inputs = {}) {
...
function sendPaginatedArray(req, res, items) {
    const { shouldPaginate, page, limit, offset } = getPaginationParams(req);
    if (!shouldPaginate) {
        res.json(items);
        return;
...
async function generateDocNumber(docType, tenantId) {
    const defaults = {
    ...
    if (!seq) {
        ...
        if (!fallback)
            throw new Error(`Document sequence not found: ${docType}`);
        const id = uuidv4();
    ...
    while (true) {
        ...
        const existingResult = await runRawQuery(`SELECT TOP 1 id FROM dbo.${tableName} WHERE ${identifierColumn} = @candidate AND tenant_id = @tenant_id`, { candidate, tenant_id: tenantId });
...
// ===== HELPER: Update stock =====
// ===== HELPER: Update stock =====
async function updateStock(tenantId, itemId, qty, movementType, refId, refType, cost = 0, userId) {
...
async function adjustWarehouseStock(tenantId, warehouseId, itemId, delta) {
    if (!warehouseId || !itemId || !Number.isFinite(delta) || delta === 0)
        return;
...
async function setWarehouseStock(tenantId, warehouseId, itemId, quantity) {
    if (!warehouseId || !itemId || !Number.isFinite(quantity))
        return;
    const existing = await db.query `
    SELECT TOP 1 * FROM warehouse_stock
    WHERE warehouse_id = ${warehouseId} AND item_id = ${itemId} AND tenant_id = ${tenantId}
...
async function computeDerivedAccountBalances(accounts, tenantId) {
    const tenantFilter = tenantId || null;
    const [invoices, bills, paymentsReceived, paymentsMade, expenses, creditNotes, vendorCredits, salesReturns, purchaseReturns, items,] = await Promise.all([
    ...
    const directExpenses = expenses.reduce((sum, row) => sum + Number(row.amount || 0) + Number(row.tax_amount || 0), 0);
    const cashPosition = sumField(paymentsReceived, "amount") - sumField(paymentsMade, "amount") - directExpenses;
    const inventoryValue = items.reduce((sum, row) => sum + (Number(row.current_stock || 0) * Number(row.purchase_rate || 0)), 0);
    ...
    for (const account of accounts) {
        ...
        let derived = null;
        if (type === "asset") {
            if (/(receivable|debtor|customer)/.test(name))
                derived = receivables;
            else if (/(inventory|stock)/.test(name))
                derived = inventoryValue;
            else if (/(cash|bank|upi|wallet|card)/.test(name))
                derived = cashPosition;
        }
        else if (type === "liability") {
            if (/(payable|creditor|vendor|supplier)/.test(name))
                derived = payables;
        }
        else if (type === "income") {
            if (/(sale|revenue|income)/.test(name))
                derived = netSales;
        }
            ...
            if (matchedCategory)
                derived = matchedCategory[1];
            else if (/(purchase|cost of goods|cogs)/.test(name))
                derived = purchaseSpend;
            else if (/(expense|overhead|admin|rent|salary|travel|utility|office|marketing|fuel|repair|misc)/.test(name))
                derived = directExpenses;
        }
        else if (type === "equity") {
            if (/(equity|capital|retained|earning|owner)/.test(name))
                derived = equityValue;
        }
        derivedById.set(account.id, derived);
    }
    for (const [type, total] of Object.entries(typeTotals)) {
        const accountsOfType = accounts.filter((account) => String(account.account_type || "").toLowerCase() === type);
        if (accountsOfType.length === 0)
            continue;
        const allocated = accountsOfType.reduce((sum, account) => sum + Number(derivedById.get(account.id) || 0), 0);
        const residual = total - allocated;
        if (Math.abs(residual) <= 0.001)
            continue;
        const target = accountsOfType.find((account) => derivedById.get(account.id) == null) || accountsOfType[0];
...
// ===== CUSTOMERS =====
router.get("/customers", authenticate, requireTenantContext, requireFeatureAccess("sales.customers"), async (req, res) => {
    try {
...
router.post("/customers", authenticate, requireTenantContext, requireFeatureAccess("sales.customers"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const id = uuidv4();
...
router.put("/customers/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const { name, email, phone, gstin, pan, billing_address, shipping_address, state, credit_limit, outstanding_balance, is_active } = req.body;
...
router.delete("/customers/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        ...
        if (message.includes("DELETE statement conflicted") ||
...
router.get("/customers/:id/ledger", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
// ===== VENDORS =====
router.get("/vendors", authenticate, requireTenantContext, requireFeatureAccess("purchase.vendors"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        await sendPaginatedResults(req, res, `
      SELECT
...
router.get("/vendors/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
router.post("/vendors", authenticate, requireTenantContext, requireFeatureAccess("purchase.vendors"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const id = uuidv4();
...
router.put("/vendors/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const { name, email, phone, gstin, pan, address, state, outstanding_balance, is_active } = req.body;
...
// ===== ITEMS =====
router.get("/items", authenticate, requireTenantContext, requireFeatureAccess("inventory.items"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        await sendPaginatedResults(req, res, `
...
router.get("/items/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
router.post("/items", authenticate, requireTenantContext, requireFeatureAccess("inventory.items"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const id = uuidv4();
...
router.put("/items/:id", authenticate, requireTenantContext, requireFeatureAccess("inventory.items"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
router.delete("/items/:id", authenticate, requireTenantContext, requireFeatureAccess("inventory.items"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
router.get("/items/:id/stock", authenticate, requireTenantContext, requireFeatureAccess("inventory.stock_ledger"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
// ===== JOURNAL ENTRIES =====
router.get("/journal-entries", authenticate, requireTenantContext, requireFeatureAccess("accounting.journal_entries"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const { shouldPaginate, page, limit, offset } = getPaginationParams(req);
...
router.get("/journal-entries/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
router.post("/journal-entries", authenticate, requireTenantContext, requireFeatureAccess("accounting.journal_entries"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const id = uuidv4();
...
router.delete("/journal-entries/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
// ===== ACCOUNTS =====
router.get("/accounts", authenticate, requireTenantContext, requireFeatureAccess("accounting.chart_of_accounts"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
router.get("/accounts/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const rawAccounts = await db.query `SELECT * FROM accounts WHERE tenant_id = ${tenantId} ORDER BY code`.then(res => res.recordset);
        const data = (await computeDerivedAccountBalances(rawAccounts, tenantId)).find((account) => account.id === req.params.id);
        if (!data) {
            res.status(404).json({ error: "Not found" });
            return;
        }
...
router.post("/accounts", authenticate, requireTenantContext, requireFeatureAccess("accounting.chart_of_accounts"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const id = uuidv4();
...
router.put("/accounts/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const { code, name, account_type, parent_id, is_system, balance } = req.body;
...
router.delete("/accounts/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
// ===== STOCK MOVEMENTS =====
router.get("/stock-movements", authenticate, requireTenantContext, requireFeatureAccess("inventory.stock_ledger"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        await sendPaginatedResults(req, res, `
      SELECT
...
router.get("/stock-movements/:id", authenticate, requireTenantContext, requireFeatureAccess("inventory.stock_ledger"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
router.post("/gst-settings", authenticate, requireTenantContext, requireFeatureAccess("gst.gst_settings"), async (req, res) => {
    ...
    `;
        ...
        const { gstin, legal_name, trade_name, state, state_code, is_composition, reverse_charge_applicable, einvoice_enabled, eway_bill_enabled, } = req.body;
        let data;
        if (existing) {
            await db.query `
        UPDATE gst_settings
...
// ===== DOCUMENT SEQUENCES =====
router.get("/document-sequences", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
router.put("/document-sequences/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        if (!requireAdmin(req, res))
            return;
        const tenantId = getTenantId(req);
...
router.post("/company", authenticate, requireTenantContext, async (req, res) => {
    try {
        if (!requireAdmin(req, res))
            return;
        const tenantId = getTenantId(req);
...
router.get("/user-roles", authenticate, async (req, res) => {
    ...
    `.then(res => res.recordset);
        res.json(rows.map((row) => ({
            ...
            is_cybaemtech_team: String(row.tenant_id || "") === String(cybaemtechTenantId || ""),
        })));
    }
    catch (e) {
        res.status(500).json({ error: e.message });
...
router.post("/super-admin/companies", authenticate, async (req, res) => {
    try {
        if (!(await requireCybaemtechSuperAdmin(req, res)))
            return;
        const { company_name, gstin, address, company_email, admin_name, admin_email, admin_password, admin_phone, plan_name, invoice_limit, user_limit, start_date, end_date, auto_renew, } = req.body || {};
...
router.get("/item-categories/:id", authenticate, requireTenantContext, requireFeatureAccess("inventory.categories"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
router.delete("/item-categories/:id", authenticate, requireTenantContext, requireFeatureAccess("inventory.categories"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
// ===== PRICE LISTS =====
router.get("/price-lists", authenticate, requireTenantContext, requireFeatureAccess("inventory.price_lists"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        await sendPaginatedResults(req, res, `
      SELECT
...
router.get("/price-lists/:id", authenticate, requireTenantContext, requireFeatureAccess("inventory.price_lists"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
router.post("/price-lists", authenticate, requireTenantContext, requireFeatureAccess("inventory.price_lists"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const id = uuidv4();
        ...
        `;
                if (!insertResult.rowsAffected[0])
                    throw new Error("Price list item does not belong to this organization");
            }
        }
...
router.delete("/price-lists/:id", authenticate, requireTenantContext, requireFeatureAccess("inventory.price_lists"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
// ===== WAREHOUSES =====
router.get("/warehouses", authenticate, requireTenantContext, requireFeatureAccess("inventory.warehouses"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        await sendPaginatedResults(req, res, `
      SELECT
...
router.get("/warehouses/:id", authenticate, requireTenantContext, requireFeatureAccess("inventory.warehouses"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
router.post("/warehouses", authenticate, requireTenantContext, requireFeatureAccess("inventory.warehouses"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const id = uuidv4();
...
router.delete("/warehouses/:id", authenticate, requireTenantContext, requireFeatureAccess("inventory.warehouses"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
router.get("/inventory-adjustments", authenticate, requireTenantContext, requireFeatureAccess("inventory.adjustments"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        await sendPaginatedResults(req, res, `
      SELECT
...
router.get("/inventory-adjustments/:id", authenticate, requireTenantContext, requireFeatureAccess("inventory.adjustments"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
router.post("/inventory-adjustments", authenticate, requireTenantContext, requireFeatureAccess("inventory.adjustments"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
// ===== STOCK TRANSFERS =====
router.get("/stock-transfers", authenticate, requireTenantContext, requireFeatureAccess("inventory.stock_transfers"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        await sendPaginatedResults(req, res, `
      SELECT
...
router.get("/stock-transfers/:id", authenticate, requireTenantContext, requireFeatureAccess("inventory.stock_transfers"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
router.post("/stock-transfers", authenticate, requireTenantContext, requireFeatureAccess("inventory.stock_transfers"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
router.get("/pos/sessions/:id", authenticate, requireTenantContext, requireFeatureAccess("pos.sessions"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
router.post("/pos/sessions", authenticate, requireTenantContext, requireFeatureAccess("pos.sessions"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const id = uuidv4();
...
router.put("/pos/sessions/:id/close", authenticate, requireTenantContext, requireFeatureAccess("pos.sessions"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
router.get("/pos/orders/:id", authenticate, requireTenantContext, requireFeatureAccess("pos.orders"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
router.post("/pos/orders", authenticate, requireTenantContext, requireFeatureAccess("pos.orders"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const id = uuidv4();
...
// ===== RECURRING INVOICES =====
router.get("/recurring-invoices", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        await sendPaginatedResults(req, res, `
...
router.get("/recurring-invoices/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const dataResult = await db.query `
      SELECT
...
router.post("/recurring-invoices", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const id = uuidv4();
...
router.delete("/recurring-invoices/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
// ===== RECURRING BILLS =====
router.get("/recurring-bills", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        await sendPaginatedResults(req, res, `
...
router.get("/recurring-bills/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const dataResult = await db.query `
      SELECT
...
router.post("/recurring-bills", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const id = uuidv4();
...
router.delete("/recurring-bills/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
// ===== INVOICES =====
router.get("/invoices", authenticate, requireTenantContext, requireFeatureAccess("sales.invoices"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        await sendPaginatedResults(req, res, `
...
router.get("/invoices/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const dataResult = await db.query `
...
router.post("/invoices", authenticate, requireTenantContext, requireFeatureAccess("sales.invoices"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const id = uuidv4();
...
router.put("/invoices/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const { customer_id, date, due_date, sales_order_id, reference_id, reference_type, status, subtotal, tax_amount, total, balance_due, notes, terms, items } = req.body;
...
router.patch("/invoices/:id/status", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
// ===== PURCHASE ORDERS =====
router.get("/purchase-orders", authenticate, requireTenantContext, requireFeatureAccess("purchase.purchase_orders"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        await sendPaginatedResults(req, res, `
...
router.get("/purchase-orders/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const dataResult = await db.query `
...
router.post("/purchase-orders", authenticate, requireTenantContext, requireFeatureAccess("purchase.purchase_orders"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const id = uuidv4();
...
router.put("/purchase-orders/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const { vendor_id, date, expected_delivery, reference_id, reference_type, status, subtotal, tax_amount, total, notes, items } = req.body;
...
router.patch("/purchase-orders/:id/status", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const { status } = req.body;
...
router.delete("/purchase-orders/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
// ===== BILLS =====
router.get("/bills", authenticate, requireTenantContext, requireFeatureAccess("purchase.bills"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        await sendPaginatedResults(req, res, `
...
router.get("/bills/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const dataResult = await db.query `
      SELECT b.*, v.name as vendor_name, v.gstin as vendor_gstin, v.address as vendor_address, v.state as vendor_state 
...
router.post("/bills", authenticate, requireTenantContext, requireFeatureAccess("purchase.bills"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const id = uuidv4();
...
router.put("/bills/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const { vendor_id, date, due_date, purchase_order_id, reference_id, reference_type, status, subtotal, tax_amount, total, balance_due, notes, items } = req.body;
...
router.patch("/bills/:id/status", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const { status } = req.body;
...
router.delete("/bills/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
// ===== CREDIT NOTES =====
router.get("/credit-notes", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        await sendPaginatedResults(req, res, `
...
router.get("/credit-notes/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const dataResult = await db.query `
      SELECT cn.*, c.name as customer_name, c.gstin as customer_gstin, c.billing_address as customer_address, c.state as customer_state 
...
router.post("/credit-notes", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const id = uuidv4();
...
router.put("/credit-notes/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const { customer_id, date, invoice_id, reference_id, reference_type, status, subtotal, tax_amount, total, reason, items } = req.body;
...
router.delete("/credit-notes/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        await db.query `DELETE FROM credit_note_items WHERE credit_note_id = ${req.params.id} AND tenant_id = ${tenantId}`;
...
// ===== VENDOR CREDITS =====
router.get("/vendor-credits", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        await sendPaginatedResults(req, res, `
...
router.get("/vendor-credits/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const dataResult = await db.query `
...
router.post("/vendor-credits", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const id = uuidv4();
...
router.put("/vendor-credits/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const { vendor_id, date, bill_id, reference_id, reference_type, status, subtotal, tax_amount, total, reason, items } = req.body;
...
router.delete("/vendor-credits/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        await db.query `DELETE FROM vendor_credit_items WHERE vendor_credit_id = ${req.params.id} AND tenant_id = ${tenantId}`;
...
// ===== QUOTATIONS =====
router.get("/quotations", authenticate, requireTenantContext, requireFeatureAccess("sales.quotations"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        await sendPaginatedResults(req, res, `
...
router.get("/quotations/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const dataResult = await db.query `
...
router.post("/quotations", authenticate, requireTenantContext, requireFeatureAccess("sales.quotations"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const id = uuidv4();
...
router.put("/quotations/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const { customer_id, date, valid_until, reference_id, reference_type, status, subtotal, tax_amount, total, notes, terms, items } = req.body;
...
router.patch("/quotations/:id/status", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const { status } = req.body;
...
router.delete("/quotations/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
// ===== SALES ORDERS =====
router.get("/sales-orders", authenticate, requireTenantContext, requireFeatureAccess("sales.sales_orders"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        await sendPaginatedResults(req, res, `
...
router.get("/sales-orders/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const dataResult = await db.query `
      SELECT so.*, c.name as customer_name, c.gstin as customer_gstin, c.billing_address as customer_address 
...
router.post("/sales-orders", authenticate, requireTenantContext, requireFeatureAccess("sales.sales_orders"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const id = uuidv4();
...
router.put("/sales-orders/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const { customer_id, date, expected_delivery, quotation_id, reference_id, reference_type, status, subtotal, tax_amount, total, notes, items } = req.body;
...
router.patch("/sales-orders/:id/status", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const { status } = req.body;
...
router.delete("/sales-orders/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
// ===== SALES RETURNS =====
router.get("/sales-returns", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        await sendPaginatedResults(req, res, `
...
router.get("/sales-returns/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const dataResult = await db.query `
...
router.post("/sales-returns", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const id = uuidv4();
...
router.put("/sales-returns/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const { customer_id, invoice_id, date, subtotal, tax_amount, total, reason, notes, status, items } = req.body;
...
router.delete("/sales-returns/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        await db.query `DELETE FROM sales_return_items WHERE sales_return_id = ${req.params.id} AND tenant_id = ${tenantId}`;
...
// ===== PURCHASE RETURNS =====
router.get("/purchase-returns", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        await sendPaginatedResults(req, res, `
...
router.get("/purchase-returns/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const dataResult = await db.query `
...
router.post("/purchase-returns", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const id = uuidv4();
...
router.put("/purchase-returns/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const { vendor_id, bill_id, date, subtotal, tax_amount, total, reason, notes, status, items } = req.body;
...
router.delete("/purchase-returns/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        await db.query `DELETE FROM purchase_return_items WHERE purchase_return_id = ${req.params.id} AND tenant_id = ${tenantId}`;
...
router.post("/workflows", authenticate, requireTenantContext, requireFeatureAccess("automation.workflows"), async (req, res) => {
    try {
        const id = uuidv4();
...
// ===== BANK ACCOUNTS =====
router.get("/bank-accounts", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
router.post("/bank-accounts", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const id = uuidv4();
...
// ===== EXPENSES =====
router.get("/expenses", authenticate, requireTenantContext, requireFeatureAccess("inventory.expenses"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        await sendPaginatedResults(req, res, `
...
router.post("/expenses", authenticate, requireTenantContext, requireFeatureAccess("inventory.expenses"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const id = uuidv4();
...
router.delete("/expenses/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
// ===== PAYMENTS RECEIVED =====
router.get("/payments-received", authenticate, requireTenantContext, requireFeatureAccess("sales.payments_received"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        await sendPaginatedResults(req, res, `
...
router.get("/payments-received/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const dataResult = await db.query `
...
router.post("/payments-received", authenticate, requireTenantContext, requireFeatureAccess("sales.payments_received"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const id = uuidv4();
...
router.delete("/payments-received/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
// ===== PAYMENTS MADE =====
router.get("/payments-made", authenticate, requireTenantContext, requireFeatureAccess("purchase.payments_made"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        await sendPaginatedResults(req, res, `
...
router.get("/payments-made/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const dataResult = await db.query `
...
router.post("/payments-made", authenticate, requireTenantContext, requireFeatureAccess("purchase.payments_made"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const id = uuidv4();
...
router.delete("/payments-made/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
// ===== DASHBOARD =====
router.get("/dashboard/stats", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        ...
        const totalSales = allInvoices.reduce((s, i) => s + Number(i.total), 0);
        const totalReceivables = allInvoices.reduce((s, i) => s + Number(i.balance_due), 0);
        const totalPurchase = allBills.reduce((s, b) => s + Number(b.total), 0);
        const totalPayables = allBills.reduce((s, b) => s + Number(b.balance_due), 0);
        const stockValue = allItems.reduce((s, i) => s + Number(i.current_stock) * Number(i.selling_rate), 0);
        const lowStockCount = allItems.filter((i) => Number(i.current_stock) <= Number(i.reorder_level)).length;
        const outputGst = allInvoices.reduce((s, i) => s + Number(i.tax_amount), 0);
        const inputGstFromBills = allBills.reduce((s, b) => s + Number(b.tax_amount), 0);
        const inputGstFromExpenses = allExpenses.reduce((s, e) => s + Number(e.tax_amount), 0);
...
router.get("/dashboard/recent-invoices", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const shouldPaginate = req.query.page !== undefined || req.query.limit !== undefined;
        const parsedPage = Number.parseInt(String(req.query.page ?? "1"), 10);
...
router.get("/dashboard/low-stock", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const shouldPaginate = req.query.page !== undefined || req.query.limit !== undefined;
        const parsedPage = Number.parseInt(String(req.query.page ?? "1"), 10);
...
router.get("/gst/hsn-summary", authenticate, requireTenantContext, requireFeatureAccess("gst.hsn_summary"), async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const dataResult = await db.query `
      SELECT
...
// ===== DELIVERY CHALLANS =====
router.get("/delivery-challans", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        await sendPaginatedResults(req, res, `
...
router.get("/delivery-challans/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const dataResult = await db.query `
    ...
    `.then(res => res.recordset);
        ...
        const computedTaxAmount = items.reduce((sum, item) => sum + Number(item.tax_amount || 0), 0);
...
router.post("/delivery-challans", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const id = uuidv4();
        ...
        const computedSubtotal = subtotal ?? safeItems.reduce((sum, item) => sum + Number(item.amount || (Number(item.quantity || 0) * Number(item.rate || 0))), 0);
        const computedTaxAmount = tax_amount ?? safeItems.reduce((sum, item) => sum + Number(item.tax_amount || 0), 0);
...
router.put("/delivery-challans/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const { customer_id, date, sales_order_id, reference_id, reference_type, status, notes, subtotal, tax_amount, total, items } = req.body;
        const safeItems = Array.isArray(items) ? items : [];
...
router.patch("/delivery-challans/:id/status", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const { status } = req.body;
...
router.delete("/delivery-challans/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        await db.query `DELETE FROM delivery_challan_items WHERE delivery_challan_id = ${req.params.id} AND tenant_id = ${tenantId}`;
...
// ===== SUBSCRIPTION =====
router.get("/feature-catalog", authenticate, async (_req, res) => {
    res.json({ features: FEATURE_OPTIONS });
});
router.get("/plan-features", authenticate, async (req, res) => {
    try {
        if (!(await requireCybaemtechSuperAdmin(req, res)))
            return;
        const plans = await db.query `SELECT id, name FROM plans ORDER BY name ASC`.then((result) => result.recordset);
        const payload = await Promise.all(plans.map(async (plan) => ({
            ...plan,
...
router.put("/plans/:id/features", authenticate, async (req, res) => {
    try {
        if (!(await requireCybaemtechSuperAdmin(req, res)))
            return;
        const featureKeys = Array.isArray(req.body?.featureKeys) ? req.body.featureKeys.map((item) => String(item)) : [];
        const catalogKeys = new Set(FEATURE_OPTIONS.map((feature) => feature.key));
        const invalid = featureKeys.filter((key) => !catalogKeys.has(key));
        if (invalid.length > 0) {
...
router.get("/role-permissions", authenticate, requireTenantContext, async (req, res) => {
    try {
        if (!requireAdmin(req, res))
            return;
        const tenantId = getTenantId(req);
...
router.get("/roles", authenticate, requireTenantContext, async (req, res) => {
    try {
        if (!requireAdmin(req, res))
            return;
        const tenantId = getTenantId(req);
...
router.post("/roles", authenticate, requireTenantContext, async (req, res) => {
    try {
        if (!requireAdmin(req, res))
            return;
        const tenantId = getTenantId(req);
...
router.put("/roles/:id", authenticate, requireTenantContext, async (req, res) => {
    try {
        if (!requireAdmin(req, res))
            return;
        const tenantId = getTenantId(req);
...
router.get("/access-context", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
router.put("/role-permissions/:role", authenticate, requireTenantContext, async (req, res) => {
    try {
        if (!requireAdmin(req, res))
            return;
        ...
        const allowedFeatures = await getTenantAllowedFeatures(tenantId);
        const featureKeys = Array.isArray(req.body?.featureKeys) ? req.body.featureKeys.map((item) => String(item)) : [];
        const invalid = featureKeys.filter((key) => !allowedFeatures.includes(key));
        if (invalid.length > 0) {
...
router.get("/subscription/plans", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        ...
        const liveFeatureKeysByPlan = new Map();
            ...
            plans: plans.map((plan) => {
                ...
                try {
                    ...
                    if (Array.isArray(persisted) && persisted.length > 0 && featureKeys.length === 0) {
                        featureKeys.push(...persisted.map((item) => String(item)));
                    }
                    else if (persisted && typeof persisted === "object" && Array.isArray(persisted.feature_keys) && featureKeys.length === 0) {
                        featureKeys.push(...persisted.feature_keys.map((item) => String(item)));
                    }
                }
                catch {
...
router.put("/subscription/plans/:id", authenticate, async (req, res) => {
    try {
        if (!(await requireCybaemtechSuperAdmin(req, res)))
            return;
...
router.get("/subscription/current", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
router.get("/subscription/overview", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
router.get("/subscription/organizations", authenticate, async (req, res) => {
    try {
        if (!(await requireCybaemtechSuperAdmin(req, res)))
            return;
    ...
    `.then((result) => result.recordset);
        ...
        const organizations = await Promise.all(companies.map(async (company) => {
            const companyUsers = byCompany.get(String(company.id)) || [];
            const creatorUsers = users
                .filter((user) => !user.company_id && company.created_by === user.user_id)
                .map((user) => ({
                id: user.user_id,
                email: user.email,
                username: user.username,
            ...
            const mergedUsers = [...companyUsers, ...creatorUsers.filter((creator) => !companyUsers.some((user) => user.id === creator.id))];
            ...
            const roleBreakdown = mergedUsers.reduce((acc, user) => {
                const role = String(user.role || "viewer").toLowerCase();
                acc[role] = (acc[role] || 0) + 1;
                return acc;
            }, {});
            ...
            return {
                ...
                accessSummary: {
                    ...
                    activeUsers: mergedUsers.filter((user) => user.is_active).length,
...
router.put("/subscription/organizations/:id", authenticate, async (req, res) => {
    try {
        if (!(await requireCybaemtechSuperAdmin(req, res)))
            return;
        const tenantId = req.params.id;
        const { company_name, gstin, city, state, phone, email, website, address, pincode, plan_name, status, auto_renew, } = req.body || {};
        const company = await db.query `
      SELECT TOP 1 *
      FROM companies
      WHERE id = ${tenantId}
...
router.put("/subscription/organizations/:id/status", authenticate, async (req, res) => {
    try {
        if (!(await requireCybaemtechSuperAdmin(req, res)))
            return;
        const tenantId = req.params.id;
...
router.delete("/subscription/organizations/:id", authenticate, async (req, res) => {
    try {
        if (!(await requireCybaemtechSuperAdmin(req, res)))
            return;
        const tenantId = req.params.id;
...
router.put("/subscription/organizations/:id/plan", authenticate, async (req, res) => {
    try {
        if (!(await requireCybaemtechSuperAdmin(req, res)))
            return;
        const tenantId = req.params.id;
...
router.post("/billing/razorpay/order", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
...
router.post("/billing/upgrade/confirm", authenticate, requireTenantContext, async (req, res) => {
    try {
        const tenantId = getTenantId(req);
        const { planName } = req.body;
        const plan = await updateTenantPlan(tenantId, planName);
...
// ===== SOFTWARE UPDATES =====
router.get("/software/updates", authenticate, async (req, res) => {
    try {
...
router.post("/software/update", authenticate, async (req, res) => {
    try {
        if (!requireAdmin(req, res))
            return;
